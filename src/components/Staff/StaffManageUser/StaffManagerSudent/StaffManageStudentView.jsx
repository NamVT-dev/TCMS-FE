...
import StaffManageStudentDetail from './StaffManageStudentDetail';
...
const StaffManageStudentView = () => {
    ...
    const fetchTeachers = useCallback(async (currentPage, search, status) => {
        ...
        try {
            ...
            const response = await api.staff.getStudents(params);
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <div className="mb-6 flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800 mb-2">Danh sách học viên</h1>
                    <p className="text-gray-600">Quản lý thông tin học viên trong hệ thống</p>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full min-w-[900px]">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tên học viên</th>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <img
                                                    ...
                                                    src={student.profile.photo || `https://ui-avatars.com/api/?name=${student.username}&background=ede9fe&color=7c3aed`}
                                                    alt={student.username}
                                                />
                                                <div className="ml-3">
                                                    <div className="text-sm font-medium text-gray-900">{student.profile?.fullname || student.username}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.email}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.profile.dob ? new Date(student.profile.dob).toLocaleDateString('vi-VN') : 'N/A'}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.profile?.phoneNumber}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{student.profile?.gender === 'male' ? "Nam" : "Nữ"}</td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span
                                                className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadge(student.active)}`}
                                            >
                                                {student.active ? 'Hoạt động' : 'Tạm ngưng'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <div className="flex items-center justify-center space-x-3">
                                                    <Eye size={20} onClick={() => handleView(student._id)} />
                                            </div>
                                        </td>
                                    </tr>
                                ))
...
export default StaffManageStudentView;