...
const Navbar = () => {
    ...
    const [substituteRequestId, setSubstituteRequestId] = useState(null);   
    ...
    const colorMap = {
        admin: { from: 'from-purple-600', to: 'to-purple-800', border: 'border-purple-500', text: 'text-purple-700', hoverBg: 'hover:bg-purple-50', bg: 'bg-purple-100', icon: 'text-purple-600', buttonBg: 'bg-purple-600 hover:bg-purple-700', buttonBorder: 'border-purple-600 hover:bg-purple-50', buttonText: 'text-purple-700' },
        teacher: { from: 'from-purple-600', to: 'to-purple-800', border: 'border-purple-500', text: 'text-purple-700', hoverBg: 'hover:bg-purple-50', bg: 'bg-purple-100', icon: 'text-purple-600', buttonBg: 'bg-purple-600 hover:bg-purple-700', buttonBorder: 'border-purple-600 hover:bg-purple-50', buttonText: 'text-purple-700' },
        staff: { from: 'from-purple-600', to: 'to-purple-800', border: 'border-purple-500', text: 'text-purple-700', hoverBg: 'hover:bg-purple-50', bg: 'bg-purple-100', icon: 'text-purple-600', buttonBg: 'bg-purple-600 hover:bg-purple-700', buttonBorder: 'border-purple-600 hover:bg-purple-50', buttonText: 'text-purple-700' },
        member: { from: 'from-purple-600', to: 'to-purple-800', border: 'border-purple-500', text: 'text-purple-700', hoverBg: 'hover:bg-purple-50', bg: 'bg-purple-100', icon: 'text-purple-600', buttonBg: 'bg-purple-600 hover:bg-purple-700', buttonBorder: 'border-purple-600 hover:bg-purple-50', buttonText: 'text-purple-700' }
    };
    ...
    useEffect(() => {
        const handleOutsideClick = (e) => {
            if (openMenu && courseRef.current && !courseRef.current.contains(e.target) && commitmentRef.current && !commitmentRef.current.contains(e.target)) setOpenMenu(null);
            if (dropdownOpen && userDropdownRef.current && !userDropdownRef.current.contains(e.target)) setDropdownOpen(false);
            if (notiDropdownOpen && notiDropdownRef.current && !notiDropdownRef.current.contains(e.target)) setNotiDropdownOpen(false);
        };
    ...
    // 4. XỬ LÝ CLICK THÔNG BÁO
    const handleNotificationClick = (noti) => {
        setNotiDropdownOpen(false);

        if (noti.data && noti.data.linkModel === 'SubstituteRequest') {
            setSubstituteRequestId(noti.data.linkId);
        } else {
    ...
    return (
        <>
            <nav className="bg-white shadow-md sticky top-0 z-50 font-sans">
                        ...
                        <div className="flex items-center space-x-4 relative">
                            ...
                            {isLoggedIn && (
                                <div className="relative" ref={notiDropdownRef}>
                                    <button onClick={() => setNotiDropdownOpen(!notiDropdownOpen)} className={`p-2 rounded-full hover:bg-gray-100 relative ${notiDropdownOpen ? 'text-purple-600' : 'text-gray-600'}`}>
                                    ...
                                    {notiDropdownOpen && (
                                        <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white border rounded-xl shadow-xl z-50 animate-fadeIn overflow-hidden">
                                            ...
                                            <div className="max-h-[320px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200">
                                                {notifications.length === 0 ? (
                                                    ...
                                                    notifications.map((noti) => (
                                                        <div key={noti._id} onClick={() => handleNotificationClick(noti)} className={`p-4 border-b cursor-pointer hover:bg-gray-50 flex gap-3 ${!noti.isRead ? 'bg-blue-50/30' : 'bg-white'}`}>
                                                            <div className={`flex-shrink-0 w-2 h-2 mt-2 rounded-full ${!noti.isRead ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
                                                            <div className="flex-1 min-w-0">
                                                                <p className={`text-sm mb-1 truncate ${!noti.isRead ? 'font-bold' : ''}`}>{noti.title}</p>
                                                                <p className="text-xs text-gray-500 line-clamp-2">{noti.body}</p>
                                                                <p className="text-[10px] text-gray-400 mt-1 text-right">{new Date(noti.createAt).toLocaleDateString('vi-VN')}</p>
                                                            </div>
                                                        </div>
                                                    ))
                                                )}
                            ...
                            <div ref={userDropdownRef}>
                                {isLoggedIn ? (
                                        ...
                                        <div className="relative">
                                            ...
                                            {dropdownOpen && (
                                                <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-50 animate-fadeIn">
                                                    <button onClick={() => {setDropdownOpen(false); navigate('/student/profile');}} className="block w-full text-left px-4 py-2 hover:bg-gray-100">Trang cá nhân</button>
                                                    <button onClick={() => {setDropdownOpen(false); logout();}} className="block w-full text-left px-4 py-2 hover:bg-gray-100">Đăng xuất</button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center space-x-3">
                                        <button onClick={() => navigate('/login')} className="px-5 py-2 text-white bg-purple-600 rounded-full">Đăng nhập</button>
                                        <button onClick={() => navigate('/register')} className="px-5 py-2 text-purple-700 bg-white border border-purple-600 rounded-full">Đăng ký</button>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                <style>{`@keyframes fadeIn { from { opacity: 0; transform: translateY(-6px); } to { opacity: 1; transform: translateY(0); } } .animate-fadeIn { animation: fadeIn 0.2s ease-out; }`}</style>
            </nav>
            ...
            {substituteRequestId && (
                <SubstituteDetailModal 
                    isOpen={!!substituteRequestId}
                    onClose={() => setSubstituteRequestId(null)} 
                    requestId={substituteRequestId}
                />
            )}
        </>
    );
