...
import { Loader2, Save, ArrowLeft, Upload, User as UserIcon } from 'lucide-react';
import showToast from "../../../../utils/showToast";

const inputClass =
  "mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-purple-500 focus:border-purple-500 sm:text-sm";
...
const AdminStaffCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const toastId = showToast.loading(
      isEditMode ? "Đang cập nhật nhân viên..." : "Đang tạo nhân viên..."
    );
    ...
    try {
      if (isEditMode) {
        ...
        if (photoFile) updateData.append('profile[photo]', photoFile);

        await api.admin.updateStaff(id, updateData);
      } else {
        const createData = {
      ...
      showToast.updateSuccess(
        toastId,
        `${isEditMode ? "Cập nhật" : "Tạo mới"} nhân viên thành công!`
      );
      ...
      showToast.updateError(
        ...
        err?.response?.data?.message ||
          `${isEditMode ? "Cập nhật" : "Tạo mới"} nhân viên thất bại!`
      );
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <h1 className="text-3xl font-bold text-gray-800 mb-6">
        {isEditMode ? 'Cập nhật nhân viên' : 'Tạo mới nhân viên'}
      </h1>
            ...
            <div>
              ...
              {!isEditMode && (
                <p className="mt-1 text-xs text-gray-500">
                  Mật khẩu tạm thời sẽ được gửi qua email này.
                </p>
              )}
            </div>
            <div>
