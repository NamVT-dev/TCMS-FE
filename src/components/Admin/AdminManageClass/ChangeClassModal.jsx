...
const Toast = ({ message, type = "success", onClose }) => {
    ...
    return (
        <div className={`fixed top-4 right-4 ${bgColor} text-white px-6 py-4 rounded-lg shadow-lg flex items-center gap-3 z-[60] animate-slide-in`}>
...
const ConfirmDialog = ({ isOpen, onClose, onConfirm, studentName, className }) => {
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md animate-scale-in">
                <div className="p-6">
                    ...
                    <p className="text-gray-600 text-center mb-6">
                        Bạn có chắc chắn muốn chuyển học viên{" "}
                        <span className="font-semibold text-purple-600">{studentName}</span>{" "}
                        sang lớp{" "}
                        <span className="font-semibold text-purple-600">{className}</span>?
                    </p>
                </div>
...
const ChangeClassModal = ({ isOpen, onClose, student, currentClass, onSuccess }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setToast({ message: "Chuyển lớp thành công!", type: "success" });
            setTimeout(() => {
                onSuccess();
                onClose();
            }, 1000);
            ...
            console.error(err);
            const errorMsg = err.response?.data?.message || "Có lỗi xảy ra khi chuyển lớp. Vui lòng kiểm tra lại sĩ số lớp mới.";
